...
//    Допустимые значения констант = [1..128]
...
#define CButtonLatchDepth   1     /* глубина защёлки интегратора, для опроса тактовых Кнопок = количество циклов опроса, в течение которых кнопка должна возвращать статус "1", чтобы посчитаться "нажатой" или "0", чтобы посчитаться "отпущенной". */
...
// Отключить обработку Энкодеров вообще (для экономии ресурсов в слабых микроконтроллерах - энкодеры же нечасто нужно обрабатывать) (чтобы отключить - раскомментируйте)
...
//    Допустимые значения параметра TIME = [1000/CButtonTimeIncFrequency .. 1000*(2^5-1)/CButtonTimeIncFrequency]  (сравниваются со значением пятибитного поля BUTTON_HOLDING_TIME).
...
//      И также, время меньше учетного порога (TIME/1000 < 1/CButtonTimeIncFrequency) приравнивается к TIME=0.
// Особенности применения:
...
typedef struct {
  volatile unsigned char* Port;  // Порт = (PORTA, PORTB,...)
  unsigned char  Pin;            // Пин = [0..7]
} TInputChannel;
...
// Кодировка сигнала в физическом канале кнопки (при простейшей реализации: один порт - одна кнопка, подтянутая к Земле "0" или к Питанию "1")
#define BUTTON_STATE    0

// Идентификаторы Энкодеров (порядковый номер в коллекции keyEncoderList):
#define ENCODER_SPEED   0    /* Примечание: Сейчас, Энкодер данной библиотекой не обрабатывается! */
